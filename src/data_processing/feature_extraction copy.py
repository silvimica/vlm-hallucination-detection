...
def extract_features(texts, vectorizer=tfidf_vectorizer, fit_vectorizer=False):
    # TF-IDF
    if fit_vectorizer:
    ...
    # Combine TF-IDF features with TTR into a single array and return
    features = np.c_[tfidf_features]
    return {"features": features}
from textblob import TextBlob
from scipy.stats import entropy

# Feature extraction function
def extract_features_new(texts, vectorizer, include_ttr=True, include_subjectivity=True, include_entropy=True, fit_vectorizer=False):
...
def apply_features(batch, tf_idf=tfidf_vectorizer):
    return extract_features_new(
        texts=batch['full_sentence'],
        vectorizer=tf_idf,
        include_ttr=True,
